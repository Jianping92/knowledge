...
地址：https://leetcode.cn/problems/remove-element/description/?envType=problem-list-v2&envId=array
...
1. 初始化指针 `k` 为0，表示下一个有效元素的位置。
2. 遍历数组，用指针 `i` 检查每个元素。
3. 当遇到不等于 `val` 的元素时，将其移动到 `nums[k]`，并递增 `k`。
4. 遍历结束后，`k` 即为有效元素的数量，数组前 `k` 个元素均为有效元素。
